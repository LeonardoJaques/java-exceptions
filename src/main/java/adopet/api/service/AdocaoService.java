...
public class AdocaoService {
    ...
    public void solicitar(SolicitacaoDeAdocaoDTO dto){
        ...
        if (pet.getAdotado()){
            throw new AdocaoException("O pet já foi adotado.");
        }
        ...
        if (petAdocaoEmAndamento){
            throw new AdocaoException("O pet já possui uma adoção em andamento.");
        }

        Integer tutorAdocoes = adocaoRepository.countByTutorIdAndStatus(dto.idTutor(), StatusAdocao.APROVADO);
        if (tutorAdocoes == 2) {
            throw new AdocaoException("O tutor com o maximo de adoções aprovadas.");
        }
